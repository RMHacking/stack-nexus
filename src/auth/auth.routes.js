...
router.post('/auth/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    const st = await pool.query('SELECT banido, suspenso_ate, pendente_aprovacao FROM contas WHERE id=$1', [r.conta.id]);
    const row = st.rows[0] || {};
    ...
    if (row.pendente_aprovacao) return res.status(403).json({ erro: 'pendente', mensagem: 'Sua entrada está aguardando a aprovação do Fundador. Você recebe acesso assim que ele confirmar.' });
